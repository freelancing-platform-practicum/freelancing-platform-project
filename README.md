...
Платформа может помочь IT специалистам создать удобную и эффективную среду
для нахождения работы и развития профессиональной карьеры в IT. 
Может помочь сократить время и усилия, связанные с поиском проектов
и коммуникацией с заказчиками.
...
Redoc и Swagger доступны по следующим адресам (есть проблемы для `/users` и `/login`,
поэтому смотри [Эндпоинты модели Member](#эндпоинты-модели-member)):
...
2. Пользователь получает на свой электронный адрес письмо, в теле которого есть ссылка
   в формате `имя_сайта/api/users/reset_password_confirm/{uid}/{token}/`,
